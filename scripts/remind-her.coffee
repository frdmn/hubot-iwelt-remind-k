...
class Reminders
  ...
  fire: (reminder, id) ->
    unless reminder.is_deleted
      if reminder.receive == 'us'
        @robot.messageRoom reminder.envelope.room, "@all " +  "#{reminder.action}"
      else
...
class ReminderAt
  ...
  constructor: (@envelope, @date, @action, @recurrent, @text, @receive) ->
  ...
  respawn: (logger) ->
    ...
    reminder = new ReminderAt @envelope, date, @action, @recurrent, @text, @receive
    if reminder.diff() <= 0
...
module.exports = (robot) ->
  ...
  robot.respond /remind (me|us) (.+) (to|for) (.*)/i, (msg) ->
    reminders.add msg
